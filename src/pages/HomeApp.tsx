...
export default function HomeApp({ user }: { user: User }) {
  ...
  useEffect(() => {
    void (async () => {
      ...
      let kids: Child[] = [];
      if (links?.length) {
        kids = links.map((row: any) => {
          const s = Array.isArray(row.students) ? row.students[0] : row.students;
          return s ? { id: s.id, full_name: s.full_name, status: s.status } : null;
        }).filter(Boolean) as Child[];
      }
      if (!kids.length && user.email) {
        const { data } = await supabase.from('students').select('id, full_name, status').ilike('email', user.email).limit(10);
        kids = (data || []).map((s) => ({ id: s.id, full_name: s.full_name, status: s.status }));
      }
      setChildren(kids);
    ...
    <div className="shell">
      ...
      {loading ? (
        ...
        <div className="glass card">
          ...
          <p className="sub" style={{ margin: 0 }}>Markaz parent_links orqali boglaydi yoki oquvchi email bir xil bolsin.</p>
        </div>
      ) : (
        <>
          {children.length > 1 && (
            <div className="glass card" style={{ padding: 12 }}>
              <select style={{ width: '100%', padding: 10, borderRadius: 10, border: '1px solid var(--border)', background: 'var(--glass-2)', color: 'inherit' }} value={activeId || ''} onChange={(e) => setActiveId(e.target.value)}>
          ...
          {tab === 'home' && (
            <>
              ...
              <div className="stats">
                ...
                <div className="glass stat"><div className="l">Tolov</div><div className="v">{payments.length}</div></div>
                <div className="glass stat"><div className="l">Xabar</div><div className="v">{messages.length}</div></div>
                <div className="glass stat"><div className="l">Test</div><div className="v">{results.length}</div></div>
              </div>
            </>
          )}
          {tab === 'attendance' && (
            <div className="glass card">
              ...
              {attendance.map((a, i) => (
                <div className="row" key={i}><span>{a.date}</span><span className={`badge ${a.status === 'present' ? 'ok' : a.status === 'absent' ? 'danger' : 'warn'}`}>{a.status}</span></div>
              ))}
            </div>
          )}
          {tab === 'payments' && (
            <div className="glass card">
              ...
              {payments.map((p, i) => (
                <div className="row" key={i}><span>{p.payment_date}</span><strong>{Number(p.amount).toLocaleString()} som</strong></div>
              ))}
            </div>
          )}
          {tab === 'messages' && (
